...
import okhttp3.Call;
import project.mvp.com.socket.adapter.NonScanningAdapter;
...
public class NonscanningActivity extends AppCompatActivity {
    @Bind(R.id.tb_main)
    Toolbar tbMain;
    ...
    private NonScanningAdapter mAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_nonscanning);
        ...
        getList();
        viewConfigura.addItemDecoration(new DividerItemDecoration(NonscanningActivity.this,DividerItemDecoration.VERTICAL));
    }
    ...
    private void getList() {
        ...
        OkHttpUtils.get().url(MyApplication.baseUrl+"api/WorkBrenchConfigs/手工?index=1&size=10000").build().execute(new StringCallback() {
            ...
            public void onResponse(String response) {
                ...
                mAdapter = new NonScanningAdapter(NonscanningActivity.this, mWorkConfig.getDatas());
                viewConfigura.setLayoutManager(new LinearLayoutManager(NonscanningActivity.this));
                viewConfigura.setAdapter(mAdapter);

                mAdapter.setClickListener(new NonScanningAdapter.onItemClickListener() {
                    @Override
                    public void clickItem(int position,int state) {
                        Intent intent = new Intent(NonscanningActivity.this, WareActivity.class);
                        intent.putExtra("productID", mWorkConfig.getDatas().get(position).getId());
                        startActivity(intent);
                    }
                });

                mAdapter.setDeleteProductTask(new NonScanningAdapter.deleteProductTask() {
                    @Override
